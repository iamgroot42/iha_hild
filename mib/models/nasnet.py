"""
    Largely borrowed from https://github.com/weiaicunzai/pytorch-cifar100/blob/master/models/nasnet.py
"""
...
class NormalCell(nn.Module):
    ...
    def forward(self, x):
        ...
        return ch.cat([prev, x1, x2, x3, x4, x5], 1), x
...
class NasNetA(nn.Module):
    ...
    def forward(self, x):
        x = self.stem(x)
        x, prev = self.cell_layers((x, None))
        x = self.relu(x)
        x = self.avg(x)
        x = x.view(x.size(0), -1)
        x = self.fc(x)
